       ...
       <div class="container">
            <nav aria-label="breadcrumb">
               <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item"><a href="/garena/lien-quan" title="{{ $category->title }}">{{ $category->title }}</a></li>
                  {{-- <li class="breadcrumb-item active" aria-current="page">{{ $nick->code }}</li> --}}
               </ol>
            </nav>
          <div class="row">
             <div class="alert alert-info" role="alert">
               <h2 class="alert-heading">Danh mục game {{ $category->title }}</h2>
               <p></p><p><span style="color:#e74c3c"><strong>Dịch Vụ {{ $category->title }}: </strong></span><strong>{{ $category->description }}</strong></p><p></p>
           </div>
       ...
       <div class="m-l-10 m-r-10">
          ...
          <div class="c-content-client-logos-slider-1  c-bordered" data-slider="owl">
             ...
             <div class="c-content-title-1">
                <h3 class="c-center c-font-uppercase c-font-bold">Danh mục game {{ $category->title }}</h3>
                ...
                <div class="col-sm-3 col-xs-6 p-5">
                   <div class="classWithPad">
                      <div class="news_image">
                         ...
                         <a href="{{route('danhmuccon', $category->slug)}}" title="Danh Mục Game {{ $category->title }}" class="">
                         <img src="{{asset('uploads/category/'.$category->image)}}" alt="Danh Mục Game {{ $category->title }}"></a>
                      </div>
                      <div class="news_title">
                         <h2>
                            <a href="{{route('danhmuccon', $category->slug)}}" title="Danh Mục Game {{ $category->title }}">Danh Mục Game {{ $category->title }}</a>
                         </h2>
                      </div>
                      <div class="news_description">
                         <p>
                            Số tài khoản: {{ $category->nick_count }}
                         </p>
                         <div class="a-more">
                           <div class="row">
                               <div class="col-xs-12">
                                   <div class="custom72 view">
                                       <a href="{{ route('danhmucgame', [$category->slug]) }}" class=""
